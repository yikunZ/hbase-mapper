        ...
        try {
            switch (className) {
                ...
                case "java.time.LocalTime":
                    return LocalTime.ofNanoOfDay(Bytes.toLong(bytes));
    ...
    public byte[] javaObject2Bytes(Object data) {
        ...
        try {
            switch (type) {
                ...
                case "java.time.LocalDate":
                    return Bytes.toBytes(((LocalDate) data)
                            .atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli());

                case "java.time.LocalTime":
                    return Bytes.toBytes(((LocalTime) data).toNanoOfDay());

                case "java.time.LocalDateTime":
                    return Bytes.toBytes(((LocalDateTime) data)
